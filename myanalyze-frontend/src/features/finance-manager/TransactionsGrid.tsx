...
function TransactionsGrid({ kind, rows, onAdd, onEdit, onDelete, onRefresh, plannedIncomes = [], onIncomeRefresh, allIncomes, allExpenses, onAllRefresh }: TransactionsGridProps) {
  ...
  const columns = React.useMemo<DataGridColumn<TransactionRow>[]>(() => [
    ...
    { key: "status", label: "Status", value: (row) => transactionStatus(row).value, render: (row) => { const status = transactionStatus(row); return <ModuleBadge tone={status.tone} size="sm">{status.label}</ModuleBadge>; }, sortable: true, filterable: true, filterOptions: ["Zaplanowane", "Zrealizowane", "Oczekujące bankowe", "Anulowane bankowe"], width: 170, align: "center" },
  ], [accountNames, accountOptions, allTransactionMap, customTypes, isIncome, kind]);
  ...
  return (
    <div className="min-w-0 space-y-3">
      ...
      <Modal open={Boolean(editor)} onClose={() => setEditor(null)} title={editor?.mode === "edit" ? `Edytuj: ${singular}` : `Dodaj: ${singular}`} size="lg">
