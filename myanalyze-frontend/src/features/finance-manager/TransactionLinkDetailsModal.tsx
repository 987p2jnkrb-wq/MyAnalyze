...
export default function TransactionLinkDetailsModal({ open, sourceKind, source, counterpartKind, counterpart, accountNames, onClose, onChange, onUnlink, preview = false, onConfirm, previewMessage, confirmLabel = "Powiąż jako transfer własny" }: { open: boolean; sourceKind: TransactionKind; source: LinkDisplayTransaction | null; counterpartKind: TransactionKind | null; counterpart: LinkDisplayTransaction | null; accountNames: Map<number, string>; onClose: () => void; onChange: () => void; onUnlink: () => void; preview?: boolean; onConfirm?: () => void; previewMessage?: string; confirmLabel?: string }) {
  ...
  const footer = !source ? undefined : preview
    ? <div className="flex flex-wrap justify-end gap-2"><Button tone="neutral" onClick={onClose}>Anuluj</Button><Button tone="primary" onClick={onConfirm}><Link2 size={17} aria-hidden="true" />{confirmLabel}</Button></div>
    : <div className="flex flex-wrap justify-end gap-2"><Button tone="neutral" onClick={onClose}>Zamknij</Button><Button tone="secondary" onClick={onChange}><Link2 size={17} aria-hidden="true" />Zmień powiązanie</Button><Button tone="danger" onClick={onUnlink}><Unlink2 size={17} aria-hidden="true" />Usuń powiązanie</Button></div>;
  return <Modal open={open} onClose={onClose} title={preview ? "Porównaj sugerowany transfer" : "Powiązanie transferu własnego"} size="lg" footer={footer}>
