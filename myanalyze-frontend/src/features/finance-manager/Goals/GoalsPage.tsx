...
import {
  allocateNewFunds, applyGoalAccountBalances, buildBufferStatus, buildGoalMetric, buildGoalProjection, buildGoalRecommendations, buildGoalsAiPrompt, buildProposedGoals, proposedGoalAlreadyExists,
  DEFAULT_GOAL_SETTINGS, goalPayload, goalSettingsPayload, normalizeFinancialGoal, normalizeGoalSettings, validateFinancialGoal, validateGoalSettings,
  type AllocationLine, type FinancialGoal, type FinancialGoalDraft, type GoalMetric, type GoalPriority, type GoalSettings, type GoalStatus, type GoalType,
} from "./goalsModel";
...
function GoalForm({ initial, accounts, saving, onCancel, onSave }: { initial: FinancialGoalDraft; accounts: ReturnType<typeof useAccountContext>["accounts"]; saving: boolean; onCancel: () => void; onSave: (goal: FinancialGoalDraft) => Promise<void> }) {
  ...
  return <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); submit(); }}>
    ...
    <div className="flex justify-end gap-2 border-t border-slate-200 pt-4"><button type="button" disabled={saving} className="rounded-lg border border-slate-300 bg-white px-4 py-2 font-semibold" onClick={onCancel}>Anuluj</button><button type="submit" disabled={saving} className="rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white disabled:opacity-50">{saving ? "Zapisywanie…" : "Zapisz cel"}</button></div>
  </form>;
}

function SettingsForm({ initial, saving, onCancel, onSave }: { initial: GoalSettings; saving: boolean; onCancel: () => void; onSave: (settings: GoalSettings) => Promise<void> }) {
  const [settings, setSettings] = React.useState(initial);
  const { showToast } = useToast();
  const updateThreshold = (index: number, value: number) => setSettings((current) => ({ ...current, thresholds: current.thresholds.map((item, itemIndex) => itemIndex === index ? value : item) as GoalSettings["thresholds"] }));
  const updateAllocation = (index: number, value: number) => setSettings((current) => ({ ...current, allocations: current.allocations.map((item, itemIndex) => itemIndex === index ? value : item) as GoalSettings["allocations"] }));
  const submit = () => {
    const validationError = validateGoalSettings(settings);
    if (validationError) return showToast(validationError, "error");
    void onSave(settings);
  };
  return <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); submit(); }}>
    <label><span className="mb-1 block text-sm font-semibold">Finansowa podłoga</span><MoneyInput min={0} max={MAX_MONEY_AMOUNT} value={settings.financialFloor} onValueChange={(value) => setSettings((current) => ({ ...current, financialFloor: numberInput(value) || 0 }))} className="w-full rounded-lg border border-slate-300 px-3 py-2.5" /></label>
    <label><span className="mb-1 block text-sm font-semibold" title="Orientacyjna kwota przeznaczona na codzienne wydatki, których nie musisz osobno planować w aplikacji. Jest używana wyłącznie do prognozowania płynności.">Budżet bieżący / dzień</span><MoneyInput min={0} max={MAX_MONEY_AMOUNT} value={settings.dailyLivingBudget} onValueChange={(value) => setSettings((current) => ({ ...current, dailyLivingBudget: numberInput(value) || 0 }))} className="w-full rounded-lg border border-slate-300 px-3 py-2.5" /><span className="mt-1 block text-xs text-slate-500">Orientacyjna rezerwa na codzienne wydatki, używana tylko w prognozie płynności.</span></label>
    <label><span className="mb-1 block text-sm font-semibold">Początek okresu wypłatowego</span><div className="flex"><input type="number" min={1} max={31} step={1} value={settings.paydayCycleStartDay} onChange={(event) => setSettings((current) => ({ ...current, paydayCycleStartDay: Number(event.target.value) }))} className="min-w-0 flex-1 rounded-l-lg border border-slate-300 px-3 py-2.5" /><span className="rounded-r-lg border border-l-0 border-slate-300 bg-slate-50 px-3 py-2.5 text-sm">dzień miesiąca</span></div><span className="mt-1 block text-xs text-slate-500">Koniec okresu przypada dzień przed kolejnym początkiem.</span></label>
    ...
    <div className="flex justify-end gap-2 border-t border-slate-200 pt-4"><button type="button" disabled={saving} className="rounded-lg border border-slate-300 bg-white px-4 py-2 font-semibold" onClick={onCancel}>Anuluj</button><button type="submit" disabled={saving} className="rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white disabled:opacity-50">Zapisz ustawienia</button></div>
  </form>;
}
...
export default function GoalsPage({ active = true }: { active?: boolean }) {
  ...
  const fetchData = React.useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      if (settingsResult.status === "fulfilled") setSettings(normalizeGoalSettings(settingsResult.value.data));
  ...
  const metrics = React.useMemo(() => new Map(goalsForAnalysis.map((goal) => [goal.id, buildGoalMetric(goal, projection.conservativeMonthlyCapacity)])), [goalsForAnalysis, projection.conservativeMonthlyCapacity]);
  ...
  const allocation = React.useMemo<AllocationLine[]>(() => allocateNewFunds(newFundsError ? 0 : numberInput(newFunds) || 0, projection.liquidity.total, settings, goalsForAnalysis, debts, projection.dailyLivingReserve), [debts, goalsForAnalysis, newFunds, newFundsError, projection.dailyLivingReserve, projection.liquidity.total, settings]);
  ...
  const columns = React.useMemo<DataGridColumn<FinancialGoal>[]>(() => [
    { key: "name", label: "Cel", value: (row) => row.name, render: (row) => <div><span className="font-semibold text-slate-900">{row.name}</span>{row.accountName && <div className="mt-1 text-xs text-slate-500">Depozyt celu: {row.accountName}{row.includeAccountBalance ? " · saldo uwzględniane" : ""}</div>}</div>, sortable: true, width: 230, edit: { type: "text", value: (row) => row.name, update: (row, value) => ({ ...row, name: String(value) }) } },
    { key: "type", label: "Typ", value: (row) => GOAL_TYPE_LABELS[row.type], render: (row) => <ModuleBadge size="sm" tone="violet">{GOAL_TYPE_LABELS[row.type]}</ModuleBadge>, sortable: true, filterable: true, width: 155 },
    ...
    { key: "feasibility", label: "Wykonalność", value: (row) => FEASIBILITY_LABELS[metrics.get(row.id)?.feasibility ?? "unknown"], render: (row) => { const feasibility = metrics.get(row.id)?.feasibility ?? "unknown"; return <ModuleBadge size="sm" tone={FEASIBILITY_TONES[feasibility]}>{FEASIBILITY_LABELS[feasibility]}</ModuleBadge>; }, sortable: true, filterable: true, width: 150 },
    { key: "priority", label: "Priorytet", value: (row) => PRIORITY_LABELS[row.priority], render: (row) => <ModuleBadge size="sm" tone={row.priority === "high" ? "warning" : "neutral"}>{PRIORITY_LABELS[row.priority]}</ModuleBadge>, sortable: true, filterable: true, width: 125, edit: { type: "select", value: (row) => row.priority, options: Object.entries(PRIORITY_LABELS).map(([value, label]) => ({ value, label })), update: (row, value) => ({ ...row, priority: value as GoalPriority }) } },
    { key: "status", label: "Status", value: (row) => STATUS_LABELS[row.status], render: (row) => <ModuleBadge size="sm" tone={row.status === "active" ? "info" : row.status === "completed" ? "success" : "neutral"}>{STATUS_LABELS[row.status]}</ModuleBadge>, sortable: true, filterable: true, filterOptions: Object.values(STATUS_LABELS), width: 140, edit: { type: "select", value: (row) => row.status, options: Object.entries(STATUS_LABELS).map(([value, label]) => ({ value, label })), update: (row, value) => ({ ...row, status: value as GoalStatus }) } },
  ], [analysisGoalsById, metrics]);
  ...
  const saveSettings = async (next: GoalSettings) => {
    setSaving(true);
    try { await apiClient.put("/financial-goal-settings/1", goalSettingsPayload(next)); setSettings(next); setSettingsOpen(false); showToast("Zapisano ustawienia planowania.", "success"); }
  ...
  return <div className="min-w-0 space-y-5">
    ...
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><h2 className="text-lg font-bold text-slate-900">Co zrobić z nowymi środkami?</h2><p className="mb-3 text-sm text-slate-500">Kalkulator tworzy sugestię. Nie zmienia sald, celów ani zobowiązań.</p><label><span className="mb-1 block text-sm font-semibold">Mam dodatkowe</span><MoneyInput min={0} max={MAX_MONEY_AMOUNT} value={newFunds} onValueChange={setNewFunds} aria-invalid={Boolean(newFundsError)} className={`w-full rounded-lg border px-3 py-2.5 ${newFundsError ? "border-red-400" : "border-slate-300"}`} /></label>{newFundsError && <p className="mt-1 text-sm text-red-600">{newFundsError}</p>}{!newFundsError && numberInput(newFunds) > 0 && <div className="mt-3 space-y-2">{allocation.map((line, index) => <div key={`${line.kind}-${line.goalId ?? line.debtId ?? index}`} className={`flex items-center justify-between gap-3 rounded-lg px-3 py-2 ${line.kind === "mortgage" ? "border border-slate-200 bg-white" : "bg-slate-50"}`}><span className="text-sm text-slate-700">{line.label}</span><strong>{formatCurrency(line.amount)}</strong></div>)}</div>}</section>
    </div>

    <Modal open={Boolean(editor)} onClose={() => { if (!saving) setEditor(null); }} title={editor?.id == null ? "Dodaj cel" : "Edytuj cel"} description="Cel nie wykonuje przelewów i nie zmienia salda depozytu." size="lg">{editor && <GoalForm key={`${editor.id ?? "new"}-${editor.draft.name}`} initial={editor.draft} accounts={accounts} saving={saving} onCancel={() => setEditor(null)} onSave={saveGoal} />}</Modal>
    <Modal open={settingsOpen} onClose={() => { if (!saving) setSettingsOpen(false); }} title="Ustawienia planowania" description="Finansowa podłoga, budżet bieżący i strategia poduszki." size="lg"><SettingsForm initial={settings} saving={saving} onCancel={() => setSettingsOpen(false)} onSave={saveSettings} /></Modal>
