...
import {
  allocateNewFundsWithStrategy, applyGoalAccountBalances, buildBufferStatus, buildGoalMetric, buildGoalProjection, buildGoalRecommendations, buildGoalsAiPrompt, buildProposedGoals, buildSuggestedNewFundsStrategy, newFundsStrategyPayload, normalizeNewFundsStrategy, proposedGoalAlreadyExists,
...
function SettingsForm({ initial, saving, onCancel, onSave, embedded = false, onChange }: { initial: GoalSettings; saving: boolean; onCancel: () => void; onSave: (settings: GoalSettings) => Promise<void>; embedded?: boolean; onChange?: (settings: GoalSettings) => void }) {
  const [settings, setSettings] = React.useState(initial);
  const { showToast } = useToast();
  React.useEffect(() => onChange?.(settings), [onChange, settings]);
  ...
  const updateAllocation = (index: number, value: number) => setSettings((current) => ({ ...current, allocations: current.allocations.map((item, itemIndex) => itemIndex === index ? Math.max(0, Math.min(100, Math.round(value))) : item) as GoalSettings["allocations"] }));
  const submit = () => {
    const validationError = validateGoalSettings(settings);
    if (validationError) return showToast(validationError, "error");
    void onSave(settings);
  };
  return <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); if (!embedded) submit(); }}>
...
export default function GoalsPage({ active = true }: { active?: boolean }) {
  ...
  const columns = React.useMemo<DataGridColumn<FinancialGoal>[]>(() => [
    ...
    { key: "feasibility", label: "Wykonalność", value: (row) => t(FEASIBILITY_LABELS[metrics.get(row.id)?.feasibility ?? "unknown"]), render: (row) => { const feasibility = metrics.get(row.id)?.feasibility ?? "unknown"; return <ModuleBadge size="sm" tone={FEASIBILITY_TONES[feasibility]}>{t(FEASIBILITY_LABELS[feasibility])}</ModuleBadge>; }, sortable: true, filterable: true, width: 150 },
  ...
  const savePlanningSettings = async () => {
    ...
    setSaving(true);
    try {
      await apiClient.put("/financial-goal-settings/1", goalSettingsPayload(planningDraft));
      await apiClient.put("/financial-goal-settings/1/new-funds-strategy", newFundsStrategyPayload(strategyDraft));
  ...
  return <div className="min-w-0 space-y-5">
    ...
    <Modal open={newFundsStrategyOpen} onClose={() => { if (!saving) setNewFundsStrategyOpen(false); }} title={t("Ustawienia planowania")} description={t("Finansowa podłoga, budżet bieżący i strategia poduszki.")} size="xl"><div className="space-y-8"><SettingsForm key={`settings-${newFundsStrategyOpen}`} initial={planningDraft ?? settings} saving={saving} embedded onChange={setPlanningDraft} onCancel={() => setNewFundsStrategyOpen(false)} onSave={savePlanningSettings} /><section className="border-t border-slate-200 pt-6"><h3 className="mb-4 text-base font-bold text-slate-900">{t("Podział nowych środków")}</h3><NewFundsStrategyForm key={`${newFundsStrategyOpen}-${newFundsStrategy ? "saved" : "suggested"}`} initial={strategyDraft ?? effectiveNewFundsStrategy} settings={planningDraft ?? settings} realLiquidity={strategyStartLiquidity} goals={goalsForAnalysis} debts={debts} accounts={activeAccounts} saving={saving} embedded onChange={setStrategyDraft} onCancel={() => setNewFundsStrategyOpen(false)} onSave={savePlanningSettings} /></section><div className="flex justify-end gap-2 border-t border-slate-200 pt-4"><button type="button" disabled={saving} className="rounded-lg border border-slate-300 bg-white px-4 py-2 font-semibold" onClick={() => setNewFundsStrategyOpen(false)}>{t("Anuluj")}</button><button type="button" disabled={saving} className="rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white disabled:opacity-50" onClick={() => void savePlanningSettings()}>{saving ? t("Zapisywanie…") : t("Zapisz ustawienia")}</button></div></div></Modal>
