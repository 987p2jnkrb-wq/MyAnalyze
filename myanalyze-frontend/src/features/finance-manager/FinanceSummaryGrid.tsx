...
export default function FinanceSummaryGrid({ view = "general", onViewChange, active = true }: { view?: FinanceSummaryView; onViewChange?: (view: FinanceSummaryView) => void; active?: boolean }) {
  ...
  const periodRows = React.useMemo<SummaryMetric[]>(() => [
    { id: "period-current", label: "Rzeczywiste środki dzisiaj", value: periodSummary.currentActual, calculation: "Suma sald rzeczywistych z Depozytów w chwili obliczenia", sectionLabel: "Podstawa okresu - wartości stałe" },
    ...
    { id: "safe-daily-budget", label: "Maksymalny bezpieczny budżet / dzień", value: periodSummary.safeDailyBudget, calculation: `Prognozowane środki po zaplanowanych wydatkach i ponad finansową podłogą, podzielone przez ${periodSummary.remainingDaysInPeriod} dni`, dailyBudgetAssessment },
    { id: "selected-income", label: "Wpływy do wybranego dnia", value: periodSummary.incomeUntilSelectedDate, calculation: "Niezrealizowane wpływy oraz przyszłe stałe wpływy do wybranej daty", sectionLabel: "Prognoza na wybrany dzień" },
    ...
    { id: "daily-living-reserve", label: "Pozostała rezerwa po wybranym dniu do kolejnej wypłaty", value: periodSummary.remainingLivingReserveAfterSelectedDate, calculation: `${periodSummary.dailyLivingBudget.toFixed(2)} × ${Math.max(0, periodSummary.remainingDaysInPeriod - periodSummary.daysToSelectedDate)} dni po wybranej dacie` },
    { id: "actual-after-living-reserve", label: "Rzeczywiste środki na dzień (uwzględniając rezerwę na codzienne wydatki)", value: periodSummary.actualAfterLivingReserveAtSelectedDate, calculation: "Rzeczywiste środki na wybrany dzień pomniejszone o narastającą rezerwę na codzienne wydatki od dzisiaj do wybranej daty" },
