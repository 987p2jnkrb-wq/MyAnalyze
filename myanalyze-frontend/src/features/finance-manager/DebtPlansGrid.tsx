...
export default function DebtPlansGrid({ active = true }: { active?: boolean }) {
  ...
  const displayRows = React.useMemo(() => filterByActiveStatus(rowsWithRecurring, statusFilter), [rowsWithRecurring, statusFilter]);
  const totals = React.useMemo(() => buildDebtPlanTotals(displayRows, recurringExpenses), [recurringExpenses, displayRows]);
  ...
  const columns = React.useMemo<DataGridColumn<DebtPlan>[]>(() => [
    ...
    { key: "kapital", label: "Zadłużenie", value: (row) => effectiveDebtPlanDebt(row, displayRows), render: (row) => formatCurrency(effectiveDebtPlanDebt(row, displayRows)), exportValue: (row) => formatCurrency(effectiveDebtPlanDebt(row, displayRows)), sortable: true, width: 145, align: "right", edit: { type: "number", min: 0, step: 0.01, value: (row) => effectiveDebtPlanCapital(row) ?? "", disabled: (row) => normalizeCreditProductType(row.typ) === "Kredyt hipoteczny" || usesCalculatedDebt(row.typ) || isCreditCardType(row.typ), update: (row, value) => updateCalculatedRow(row, { zadluzenie: Number(value) }) } },
  ...
  ], [recurringExpenses, displayRows, updateCalculatedRow]);
  ...
  return <>
    ...
    <DataGrid
      ...
      deleteSelectedConfirmMessage={(selected) => {
        ...
        return `Czy na pewno usunąć ${selected.length === 1 ? "zaznaczone zobowiązanie" : `${selected.length} zaznaczone zobowiązania`}? ${linked ? "Powiązane Kredyty i stałe wydatki również mogą zostać usunięte. Powiązane depozyty pozostaną bez zmian." : "Tej operacji nie można cofnąć."}`;
      }}
      loading={loading}
      ...
      toolbar={<>
        <ActiveStatusFilter ariaLabel="Status zobowiązań" value={statusFilter} onChange={setStatusFilter} />
    ...
    <Modal open={addOpen} onClose={() => { if (!adding) setAddOpen(false); }} title="Dodaj zobowiązanie" description="Pozycja z ratą może automatycznie utworzyć powiązany stały wydatek." size="lg" footer={<div className="flex justify-end gap-2"><button type="button" disabled={adding} className="rounded-lg border border-slate-300 bg-white px-4 py-2 font-semibold disabled:opacity-50" onClick={() => setAddOpen(false)}>Anuluj</button><button type="submit" form="debt-plan-form" disabled={adding} className="rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white disabled:opacity-50">{adding ? "Dodawanie…" : "Dodaj"}</button></div>}><DebtPlanForm recurringExpenses={recurringExpenses} creditCards={creditCards} repaymentAccounts={repaymentAccounts} onSave={add} onSavingChange={setAdding} /></Modal>
    <DebtPlanEditModal plan={editRow} onClose={() => setEditRow(null)} onSaved={fetchRows} />
    <ConfirmModal open={deleteId !== null} title="Usuń zobowiązanie" message={rows.find((row) => row.id === deleteId)?.recurring_expense_id ? "Czy usunąć zobowiązanie, odpowiadający mu Kredyt i powiązany stały wydatek?" : rows.find((row) => row.id === deleteId)?.account_id ? "Czy usunąć zobowiązanie? Powiązane konto pozostanie bez zmian." : rows.find((row) => row.id === deleteId)?.loan_id ? "Czy usunąć zobowiązanie i odpowiadający mu Kredyt?" : "Czy na pewno usunąć tę pozycję?"} confirmLabel="Usuń" onConfirm={() => void remove()} onCancel={() => setDeleteId(null)} />
    <Modal open={payId !== null} onClose={() => { if (!paying) { setPayId(null); setPayAccountId(null); } }} title="Spłać ratę" description={isInstallmentPlanType(rows.find((row) => row.id === payId)?.typ) ? `Rata ${formatCurrency(Math.min(Number(rows.find((row) => row.id === payId)?.rata_miesieczna || 0), Number(rows.find((row) => row.id === payId)?.zadluzenie || 0)))} obciąży wybrane konto i uwolni tę samą kwotę na karcie.` : `Rata ${formatCurrency(Math.min(Number(rows.find((row) => row.id === payId)?.rata_miesieczna || 0), Number(rows.find((row) => row.id === payId)?.zadluzenie || 0)))} obciąży wybrane konto i pomniejszy zadłużenie.`} size="sm" footer={<div className="flex justify-end gap-2"><button type="button" disabled={paying} className="rounded-lg border border-slate-300 bg-white px-4 py-2 font-semibold disabled:opacity-50" onClick={() => { setPayId(null); setPayAccountId(null); }}>Anuluj</button><button type="button" disabled={paying || !payAccountId} className="rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white disabled:opacity-50" onClick={() => void payInstallment()}>{paying ? "Spłacanie…" : "Spłać ratę"}</button></div>}><AllocationOptionSelect autoFocus label="Konto do spłaty" placeholder="Wybierz konto" options={accountAllocationOptions(repaymentAccounts, false)} value={payAccountId} onChange={setPayAccountId} /></Modal>
  </>;
}
