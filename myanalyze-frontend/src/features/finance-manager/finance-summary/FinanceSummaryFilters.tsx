...
export default function FinanceSummaryFilters({ accounts, accountFilter, customTypeFilters, labelFilterMode, customTypeOptions, onAccountFilterChange, onToggleCustomType, onClearCustomTypes, onLabelFilterModeChange, onClearAll }: FinanceSummaryFiltersProps) {
  ...
  return <div className="flex flex-wrap items-start gap-2 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
    <label className="text-sm font-semibold text-slate-700">Konto <select className="ml-2 rounded border border-slate-300 bg-white px-2 py-1.5 font-normal" value={accountFilter} onChange={(event) => onAccountFilterChange(event.target.value)}><option value="">Wszystkie aktywne</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.nazwa}{account.active === false ? ` (${activeStatusLabel(account.active).toLocaleLowerCase("pl-PL")})` : ""}</option>)}</select></label>
    <div ref={labelMenuRef} className="relative">
      <button type="button" aria-expanded={labelMenuOpen} className="rounded border border-slate-300 bg-white px-3 py-1.5 text-sm font-semibold text-slate-700 hover:bg-slate-50" onClick={() => setLabelMenuOpen((current) => !current)}>Etykiety: {labelFilterActive ? (labelFilterMode === "include" ? `wybrane ${customTypeFilters.length}` : `wykluczone ${customTypeFilters.length}`) : "wszystkie"}</button>
      {labelMenuOpen && <div className="absolute left-0 top-full z-40 mt-2 w-80 max-w-[calc(100vw-2rem)] rounded-lg border border-slate-200 bg-white p-3 shadow-xl">
        ...
        <div className="mt-3 max-h-56 space-y-1 overflow-y-auto border-t border-slate-100 pt-2">{customTypeOptions.length ? customTypeOptions.map(([id, name]) => <label key={id} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-sm hover:bg-slate-50"><input type="checkbox" checked={customTypeFilters.includes(id)} onChange={() => onToggleCustomType(id)} /><span>{name}</span></label>) : <p className="px-2 py-2 text-sm text-slate-500">Brak etykiet w transakcjach.</p>}</div>
